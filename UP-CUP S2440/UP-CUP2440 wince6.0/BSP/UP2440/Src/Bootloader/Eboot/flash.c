...
{
    //KITLOutputDebugString("OEMIsFlashAddr: 0x%x, %d\r\n", dwPhysStart, bRc);
    return(FALSE);
}
...
BOOL OEMStartEraseFlash(DWORD dwStartAddr, DWORD dwLength)
{
    ...
    //
    //KITLOutputDebugString( "OEMStartEraseFlash: Addr:0x%x Len:0x%x\n", dwStartAddr, dwLength);
    return(TRUE);
}
...
void OEMContinueEraseFlash(void)
{
    // Nothing to do (erase done in OEMWriteFlash)...
    //
    //KITLOutputDebugString("OEMContinueEraseFlash\r\n");
}
...
{
    //KITLOutputDebugString("OEMWriteFlash 0x%x 0x%x\r\n", dwStartAddr, dwLength);
    return(TRUE);
}
