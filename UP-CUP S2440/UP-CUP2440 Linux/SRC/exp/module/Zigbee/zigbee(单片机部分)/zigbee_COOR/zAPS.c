...
static BOOL APSGetHeader(void)
{
          ...
          (apsCurrentFrame.frameCON.bits.indirectAddressMode == APS_SOURCE_EP_PRESENT )))
    {
		...
		ConsolePutString("destEP:");//test lyj
		ConsolePutInitData(0,apsCurrentFrame.destEP);//test lyj
    }
         ...
         ((apsCurrentFrame.frameCON.Val & APS_FRAME_TYPE_MASK) == APS_FRAME_ACKNOWLEDGE ) )
    {
		...
		ConsolePutString("clusterID:");//test lyj
		ConsolePutInitData(0,apsCurrentFrame.clusterID);//test lyj
    }

    // Profile ID is now always sent.
    apsCurrentFrame.profileID.byte.LSB = APSGet();
    apsCurrentFrame.profileID.byte.MSB = APSGet();
	ConsolePutString("profileID:");//test lyj
	ConsolePutInitData(apsCurrentFrame.profileID.byte.MSB,apsCurrentFrame.profileID.byte.LSB);//test lyj
          ...
          (apsCurrentFrame.frameCON.bits.indirectAddressMode == APS_DESTINATION_EP_PRESENT )))
    {
		...
		ConsolePutString("srcEP:");//test lyj
		ConsolePutInitData(0,apsCurrentFrame.srcEP);//test lyj
    }
