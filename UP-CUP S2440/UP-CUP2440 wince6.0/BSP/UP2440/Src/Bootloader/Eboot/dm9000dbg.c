...
//    KITLOutputDebugString("DM9000: RX length byte = 0x%x.\r\n", rxlen);
...
RETAILMSG(1, (TEXT("[DM9000A]: DM9000DBG_CurrentPacketFilter()..........\r\n")));		
    ...
    KITLOutputDebugString("DM9000: Set receive packet filter [Filter=0x%x].\r\n", dwFilter);
...
BOOL DM9000DBG_MulticastList(PUCHAR pucMulticastAddresses, DWORD dwNumAddresses)
{
	...
	{
        KITLOutputDebugString("DM9000: Multicast[%d of %d]  = %x-%x-%x-%x-%x-%x\r\n",
                             (nCount + 1),
							 dwNumAddresses,
	...
	KITLOutputDebugString("DM9000: Logical Address Filter = %x.%x.%x.%x.\r\n", hash_table[3], hash_table[2], hash_table[1], hash_table[0]);
