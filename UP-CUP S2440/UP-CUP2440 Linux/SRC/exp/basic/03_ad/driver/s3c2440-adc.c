...
ssize_t s3c2440_adc_read(struct file *filp,char __user *buf,size_t count,loff_t *f_pos)
{
        int i,LOOP=10000;
...
static ssize_t s3c2440_adc_write(struct file *file, const char *buffer, size_t count, loff_t * ppos)
{
        int data;
...
//      adc_cdev.ops=&adc_fops;
        ...
        if (err < 0)
                printk(KERN_ERR "can't add s3c2440_adc");
        myclass = class_create(THIS_MODULE, "s3c2440_adc");
        if(IS_ERR(myclass)) {
          printk("Err: failed in creating class.\n");
          return -1;
        }
 
        class_device_create(myclass,NULL, MKDEV(DEVICE_MAJOR,DEVICE_MINOR), NULL, "s3c2440_adc",DEVICE_MINOR);
        ...
        printk(KERN_ERR "add s3c2440_adc ok!\n");
 
        return 0;
}
void S3C2440adc_exit(void)
{
