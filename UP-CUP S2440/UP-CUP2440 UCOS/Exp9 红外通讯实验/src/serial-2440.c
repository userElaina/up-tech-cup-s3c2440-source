...
{
	int div=s3c2440_get_bus_clk(GET_PCLK);
...
#define DefSerialN(n)	static int s3c2440_serial##n##_init(int baud){return serial_init(n,baud);}\
					static int s3c2440_serial##n##_read(void){return serial_read(n);}\
					static int s3c2440_serial##n##_write(int ch){return serial_write(n, ch);}\
					static int s3c2440_serial##n##_poll(void){return serial_poll(n);}\
					static int s3c2440_serial##n##_flush_input(void){return serial_flush_input(n);}\
					static int s3c2440_serial##n##_flush_output(void){return serial_flush_output(n);}

/* export serial driver */
#define ExportSerialN(n)	serial_driver_t s3c2440_serial##n##_driver = {	\
							s3c2440_serial##n##_init,	\
							s3c2440_serial##n##_read,\
							s3c2440_serial##n##_write,\
							s3c2440_serial##n##_poll,\
							s3c2440_serial##n##_flush_input,\
							s3c2440_serial##n##_flush_output,\
							Serial_Isr(n)}
DefSerialN(0)
DefSerialN(1)
DefSerialN(2)
