...
#define DEV485 "/dev/s3c2440_4850"
...
static void help_menu()
{
	printf("\n");
	printf("	DESCRIPTION\n");
	printf("      	S3c2440 485 uart test program. \n");	
...
int main(int argc, char **argv)
{
	...
	if(fd485 < 0){
		printf("####s3c2440_485 device open %s fail####\n",DEV485);
		return (-1);
	}
	...
	if((baud=get_baudrate(argc, argv)) == -1) {
		printf("####s3c2440 485 device baudrate set failed####\n");
	}
	...
	if(strncmp(argv[2],"send",4)==0) {
		...
		printf("####s3c2440 485 device ready to send####\n");
		#if 1
		{
			int i;
			for(i='0'; i<='z'; i++) {
		...
		printf("####s3c2440 485 device receiving ####\n");
