...
#if 1
			int i;
			char *ps0=(char*)s0, *ps1=(char*)s1;
			printf("diff:\n");
			for(i=0;i<YAFFS_BYTES_PER_CHUNK;i+=8){
				printf("%x:%x, %x:%x, %x:%x, %x:%x, %x:%x, %x:%x, %x:%x, %x:%x\n", 
					d0[i], d1[i], d0[i+1], d1[i+1], d0[i+2], d1[i+2], d0[i+3], d1[i+3], 
					d0[i+4], d1[i+4], d0[i+5], d1[i+5], d0[i+6], d1[i+6], d0[i+7], d1[i+7]);
				if(d0[i]!=d1[i]  || d0[i+1]!=d1[i+1] || d0[i+2]!=d1[i+2]  || d0[i+3]!=d1[i+3]  || 
					d0[i+4]!=d1[i+4]  || d0[i+5]!=d1[i+5]  || d0[i+6]!=d1[i+6]  || d0[i+7]!=d1[i+7])
					printf("-------------\n");
			}
			printf("spare: \n");
			printf("%x:%x, %x:%x, %x:%x, %x:%x, %x:%x, %x:%x, %x:%x\n", 
				ps0[0], ps1[0], ps0[1], ps1[1], ps0[2], ps1[2], ps0[3], ps1[3], 
				ps0[4], ps1[4], ps0[5], ps1[5], ps0[6], ps1[6], ps0[7], ps1[7] );

			printf("%x:%x, %x:%x, %x:%x, %x:%x, %x:%x, %x:%x, %x:%x\n", 
				ps0[8], ps1[8], ps0[9], ps1[9], ps0[10], ps1[10], ps0[11], ps1[11], 
				ps0[12], ps1[12], ps0[13], ps1[13], ps0[14], ps1[14], ps0[15], ps1[15] );
#endif
			return 0;
		}
