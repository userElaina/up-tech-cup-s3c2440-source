...
void MCP2510_Write(int address, int value)
{
	...
	SPISend ( MCP2510INSTR_WRITE, 0 );
	SPISend ((unsigned char)address, 0 );
	SPISend ((unsigned char)value, 0 );
...
void MCP2510_WriteBits( int address, int data, int mask )
{
	...
	SPISend ( MCP2510INSTR_BITMDFY, 0 );
	SPISend ((unsigned char)address, 0 );
	SPISend ((unsigned char)mask, 0 );
	SPISend ((unsigned char)data, 0 );
...
{   //U8 byte;
	...
	MCP2510_WriteBits(RXB0CTRL, (RXB_BUKT+RXB_RX_ANY), 0x0F);
	MCP2510_WriteBits(RXB1CTRL, RXB_RX_ANY, 0x0F);
...
//		Uart_Printf("%x\n",MCP2510_Read(i));
	...
	MCP2510_Write_Can_ID(RXM0SIDH, 0x7ff,0);
	MCP2510_Write_Can_ID(RXM1SIDH, 0x7ff,0);
	
	
	// Anyway, set all filters to 0:
	MCP2510_Write_Can_ID(RXF0SIDH, 0x7ff, 0);
	...
	MCP2510_WriteBits(RXF0SIDL,0,FILTER_MASK); 
