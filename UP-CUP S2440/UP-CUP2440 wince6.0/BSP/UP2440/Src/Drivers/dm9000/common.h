/********************************************************************************
 * 
 * $Id: common.h,v 2.0.4.2 2002/07/03 05:45:39 dvi175 Exp $
 *
 * File: Common.h
 *
...
typedef	struct _PERF_TYPE
{
	DWORD	dwLabel;
	DWORD	dwCounter;
} PERF_TYPE;

#define	MAX_PERF_LOGS	128
#define	MAX_PERF_LOG_MASK	127

#define	PROBE_ON	0x80000000
#define	PROBE_OFF	0x00000000

extern LARGE_INTEGER	_gPerfTemp;
extern int		_gnPerfLogPos;
extern PERF_TYPE	_gszPerfLogs[MAX_PERF_LOGS];
...
#define	PERF_PROBE_ON(label)	\
	do {	\
		QueryPerformanceCounter(&_gPerfTemp);\
		_gszPerfLogs[_gnPerfLogPos].dwLabel = label|PROBE_ON;	\
		_gszPerfLogs[_gnPerfLogPos].dwCounter = _gPerfTemp.LowPart;	\
		_gnPerfLogPos = (_gnPerfLogPos + 1)&MAX_PERF_LOG_MASK;	\
		} while (0)

#define	PERF_PROBE_OFF(label)	\
	do {	\
		QueryPerformanceCounter(&_gPerfTemp);\
		_gszPerfLogs[_gnPerfLogPos].dwLabel = label;	\
		_gszPerfLogs[_gnPerfLogPos].dwCounter = _gPerfTemp.LowPart;	\
		_gnPerfLogPos = (_gnPerfLogPos + 1)&MAX_PERF_LOG_MASK;	\
		} while (0)
